...
void print_stops(uint64_t *hotels, int nstops, uint64_t *stops)
{
    ...
    if (nstops > 20) {
        i = nstops-20;
        printf("printing stops in last 20 hotels:\n");
    }
    else
        printf("printing all stops:\n");
...
int main()
{
    /* testing can be done by uncommenting one hotels_test, setting nhotels
     * correctly and then uncommenting section within #######*/
    // uint64_t hotels_test[] = {
    ...
    // uint64_t hotels_test[] = {0, 130, 211, 472, 863, 900};
    // uint64_t hotels_test[] = {0, 200, 400, 600, 800};
    ...
    print_stops(hotels, nhotels, stops);
...
void longtrip(uint64_t *hotels, int nhotels, uint64_t *stops)
{
    ...
    for (U64 mask = 1; mask < total_combinations; ++mask) {
        ...
        for (int i = 0; i < nhotels; i++) {
            if (mask & (1ULL << i)) {
                uint64_t dist = hotels[i] - current;
